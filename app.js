...
import { getFirestore, doc, getDoc, collection, query, where, getDocs, setDoc, updateDoc, addDoc, deleteDoc, onSnapshot, orderBy } from "https://www.gstatic.com/firebasejs/10.8.1/firebase-firestore.js";
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                if(dados.papel === 'admin') { 
                    ...
                    document.getElementById('lms-welcome-name').innerText = `Olá, ${myUserName}!`; 
...
document.getElementById('btn-gravar-evento')?.addEventListener('click', async (e) => {
    ...
    try {
        ...
        await carregarEventosCalendario(); await carregarHorario(); 
    ...
    slot.addEventListener('click', (e) => {
        if(modoEdicaoHorario) {
            ...
            if(e.currentTarget.querySelector('.slot-event-badge')) {
                ...
                const tipoEvt = e.currentTarget.querySelector('.slot-event-badge').getAttribute('data-tipo');
                const discEvt = e.currentTarget.querySelector('.slot-event-badge').getAttribute('data-disc');
                const dReal = slotSelecionado.getAttribute('data-datareal');
                document.getElementById('mei-titulo').innerText = tituloEvt; document.getElementById('mei-tipo').innerText = tipoEvt.toUpperCase();
                document.getElementById('mei-data').innerText = dReal; document.getElementById('mei-disc').innerText = discEvt || 'N/A';
...
async function carregarHorario() {
    ...
    for(let i=0; i<5; i++) {
        ...
        mapDiasParaDataReal[diasUIAbrv[i]] = dStr;
    ...
    try {
        ...
        for(const key in horarioBase) {
            const [dataReal, hora] = key.split('_'); const disc = horarioBase[key];
            ...
            if(slot && disc) { 
                ...
                if(evtEncontrado) { slot.innerHTML += `<div class="slot-event-badge" data-titulo="${evtEncontrado.titulo}" data-tipo="${evtEncontrado.tipo}" data-disc="${evtEncontrado.disciplina}"><i class="fa-solid fa-star"></i></div>`; }
            }
        }
    } catch(err){}
}
...
async function calcularEstatisticasTurma() {
    ...
    try {
        ...
        let prhfsAtivosGerais = 0; let hPrhfPlan = 0; let hPrhfCump = 0; let alunosEmRiscoCount = 0; let totalHorasFaltasTurma = 0;
        let listaTrAlunos = [];
            ...
            try {
                ...
                notas.forEach(n => {
                    ...
                    if(val !== 'REP' && !isNaN(val)) { 
                        ...
                        if(vNum < 10) negAluno++;
                        if(matrizCurso["Sociocultural"][disc]) { sumSocio += vNum; countSocio++; } else if(matrizCurso["Científica"][disc]) { sumCient += vNum; countCient++; } else if(matrizCurso["Técnica"][disc]) { sumTec += vNum; countTec++; }
                ...
                prhfs.forEach(p => { 
                    const dataP = p.data();
                    if(dataP.status === 'ativa') prhfsAtivosGerais++; 
        ...
        document.getElementById('stat-prhf-plan').innerText = hPrhfPlan; document.getElementById('stat-prhf-cump').innerText = hPrhfCump;
        ...
        tabelaAlunos.innerHTML = listaTrAlunos.length > 0 ? listaTrAlunos.join('') : '<tr><td colspan="4" class="text-muted center">Sem dados.</td></tr>';
...
document.getElementById('btn-novo-forum')?.addEventListener('click', async () => { 
    ...
    const cList = document.getElementById('novo-forum-membros-list'); cList.innerHTML = '<p class="text-muted" style="text-size:0.8rem;">A procurar alunos...</p>';
    try {
        const qAlunos = query(collection(db, "utilizadores"), where("turma", "==", turmaAtual), where("papel", "==", "aluno"));
        const snapshot = await getDocs(qAlunos); let h = '';
        snapshot.forEach(d => { h += `<label class="membro-checkbox-item"><input type="checkbox" class="cb-membro-forum" value="${d.id}" checked> ${d.data().nome}</label>`; });
        cList.innerHTML = h;
    } catch(e) { cList.innerHTML = '<p class="text-danger">Erro.</p>'; }
});
...
document.getElementById('novo-forum-tipo')?.addEventListener('change', (e) => {
    document.getElementById('novo-forum-expira').style.display = e.target.value === 'temporario' ? 'block' : 'none';
});
...
async function carregarForuns() {
    ...
    try {
        ...
        if(res.empty) { container.innerHTML = '<p class="text-muted" style="text-align:center;">Sem fóruns criados para esta turma.</p>'; return; }
        let html = '';
        res.forEach(docSnap => {
            const f = docSnap.data(); const icon = f.tipo === 'permanente' ? 'fa-comments' : 'fa-stopwatch';
            html += `<div class="canal-card" data-id="${docSnap.id}" data-json='${JSON.stringify(f)}'><div class="canal-icon"><i class="fa-solid ${icon}"></i></div><div class="canal-info"><h4>${f.nome}</h4><p>${f.membros.length} Membro(s)</p></div></div>`;
        });
        container.innerHTML = html;
...
function iniciarChat(fId) {
    ...
    chatUnsubscribe = onSnapshot(query(collection(db, "turmas", turmaAtual, "foruns", fId, "mensagens"), orderBy("timestamp")), (snapshot) => {
        let html = '';
...
document.getElementById('btn-send-msg')?.addEventListener('click', async () => {
    const inp = document.getElementById('input-forum-msg'); const txt = inp.value.trim(); if(!txt || !forumAtivoId) return;
    try { await addDoc(collection(db, "turmas", turmaAtual, "foruns", forumAtivoId, "mensagens"), { remetente: myUserName, texto: txt, timestamp: Date.now() }); inp.value = ''; } catch(e){}
});
...
document.getElementById('btn-pauta-global')?.addEventListener('click', async () => {
    document.getElementById('modal-pauta-global').style.display = 'flex'; const container = document.getElementById('pauta-global-content'); container.innerHTML = '<p class="text-muted" style="text-align:center;">A compilar notas...</p>';
    try {
        ...
        for (const [nomeComponente, disciplinas] of Object.entries(matrizCurso)) {
            ...
            for (const [nomeDisc, modulos] of Object.entries(disciplinas)) {
                ...
                for(const mod of Object.keys(modulos)) { const nota = mapNotas[`${nomeDisc}_${mod}`] || 'SN'; let cor = "sn"; if(nota !== 'SN' && nota !== 'REP' && nota >= 10) cor = "positiva"; else if(nota === 'REP' || nota < 10) cor = "negativa"; html += `<div class="pg-nota-item"><span>${mod}</span><span class="pg-nota-val ${cor}">${nota}</span></div>`; }
                html += `</div></div>`;
            } html += `</div>`;
        } container.innerHTML = html;
...
async function carregarInfoLeitura() {
    try {
        ...
        if (docSnap.exists()) {
            ...
            ['info-aluno-idade', 'info-aluno-telemovel', 'info-aluno-email', 'info-aluno-morada', 'info-ee-nome', 'info-ee-filiacao', 'info-ee-telemovel', 'info-ee-email'].forEach(id => { const key = id.replace('info-aluno-', '').replace('info-ee-', ''); document.getElementById(id).value = d[key === 'telemovel' ? (id.includes('aluno') ? 'telAluno' : 'telEE') : (key === 'email' ? (id.includes('aluno') ? 'emailAluno' : 'emailEE') : key)] || d[id.includes('aluno') ? key : key + 'EE'] || ""; });
        }
    } catch (error) {}
}
