...
function redirecionarParaPainel(papeis) {
    ...
    // A nova porta de entrada para a Direção
...
// ==================================================
// 6. AVISO INTELIGENTE DE INSTALAÇÃO DA APP (PWA)
// ==================================================
let deferredPrompt;
...
// 1. Ouve o evento nativo (caso o browser permita no futuro)
window.addEventListener('beforeinstallprompt', (e) => {
    e.preventDefault();
    deferredPrompt = e;
});

// 2. Verifica se a pessoa JÁ ESTÁ a usar a app instalada
const isStandalone = window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone;
const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
const isAndroid = /Android/.test(navigator.userAgent);

// 3. Se NÃO estiver instalada, mostramos o banner passados 2 segundos!
if (!isStandalone && installBanner) {
    setTimeout(() => {
        installBanner.style.display = 'flex';
    }, 2000); // Espera 2 segundos para o login carregar primeiro

    if (btnInstallApp) {
        btnInstallApp.addEventListener('click', async () => {
            if (deferredPrompt) {
                // Se o browser permitir a janela automática mágica
                ...
                alert("🍎 Instalar no iPhone/iPad:\n\n1. Toque no ícone de Partilha (quadrado com seta para cima) na barra inferior.\n2. Escolha 'Adicionar ao Ecrã Principal'.");
            } else if (isAndroid) {
                // Instruções para Android
                alert("🤖 Instalar no Android:\n\n1. Toque nos 3 pontinhos no canto superior direito.\n2. Escolha 'Adicionar ao ecrã principal' ou 'Instalar Aplicação'.");
            } else {
                // Instruções para Computador
                alert("💻 Instalar no PC:\n\n1. Procure o ícone de 'Instalar' no lado direito da barra de endereço lá em cima.\n2. Ou clique nos 3 pontinhos e escolha 'Instalar Turma PRO'.");
            }
        });
    }
