...
export async function aprovarTemaPAP(alunoId, btnElement) {
    ...
    try {
        await updateDoc(doc(db, "utilizadores", alunoId), { 
            "pap.temaAprovado": true, 
            "pap.feedbackOrientador": "Tema Aprovado! Podes avançar para o desenvolvimento." 
        });
        btnElement.innerHTML = '<i class="fa-solid fa-check"></i>';
        setTimeout(() => carregarTarefasProf(), 1000);
    } catch (e) {
        btnElement.innerHTML = 'Erro';
        setTimeout(() => { btnElement.innerHTML = originalText; btnElement.disabled = false; }, 1500);
    }
}
...
export async function rejeitarTemaPAPExecutar(alunoId, motivo, btnElement) {
    ...
    try {
        await updateDoc(doc(db, "utilizadores", alunoId), { 
            "pap.tema": null, 
            "pap.feedbackOrientador": "Tema Rejeitado: " + motivo 
        });
        btnElement.innerHTML = '<i class="fa-solid fa-check"></i>';
        setTimeout(() => {
            document.getElementById('modal-rejeitar-tema-pap').style.display = 'none';
            carregarTarefasProf();
...
export async function aprovarRelatorioPAP(alunoId, btnElement) {
    ...
    try {
        await updateDoc(doc(db, "utilizadores", alunoId), { 
            "pap.relatorioAprovado": true, 
            "pap.feedbackOrientador": "Relatório Final Aprovado! Estás apto para a defesa da PAP." 
        });
        btnElement.innerHTML = '<i class="fa-solid fa-check"></i> Aprovado';
