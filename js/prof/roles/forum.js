...
export async function gerirCliquesForum(e) {
    try {
        ...
        if (e.target.closest('#btn-create-chat-prof')) { 
            console.log("Clicou em Criar Novo Chat!");
            
            const selTurmas = document.getElementById('forum-turma-select');
            if (selTurmas) {
                const turmas = state.turmasProfessor || [];
                selTurmas.innerHTML = '<option value="">Selecionar Turma...</option>' + turmas.map(t => `<option value="${t}">Turma ${t}</option>`).join('');
            }
            
            if(document.getElementById('input-nome-novo-forum')) document.getElementById('input-nome-novo-forum').value = ''; 
            ...
            if(document.getElementById('forum-bulk-actions')) document.getElementById('forum-bulk-actions').style.display = 'none'; 
        ...
        console.error("Erro interno no módulo Fórum:", error);
        return false;
    }
}
