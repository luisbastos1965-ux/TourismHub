...
export function iniciarCarrossel() {
    ...
    const startScroll = () => { 
        // 1. O SEGREDO: Desligar sempre o motor antigo antes de ligar o novo!
    ...
    // 2. Usar on... em vez de addEventListener evita que os eventos se acumulem
    track.onmouseenter = () => clearInterval(state.carouselInterval);
    track.onmouseleave = startScroll;
    track.ontouchstart = () => clearInterval(state.carouselInterval);
    track.ontouchend = startScroll;
    ...
    try {
        ...
        for (const t of state.turmasProfessor) {
            ...
            try {
                ...
                for (const docAluno of snapAl.docs) {
                    ...
                    if (faltasAluno > 6) listaAlertas.push({ id: docAluno.id, nome: docAluno.data().nome, turma: t, msg: `${faltasAluno}h de faltas injustificadas.`, tipo: 'falta' });
                    if (prhfsAluno > 1) listaAlertas.push({ id: docAluno.id, nome: docAluno.data().nome, turma: t, msg: `${prhfsAluno} PRHFs pendentes.`, tipo: 'prhf' });
                    
                    // MELHORIA D: Aniversários
                    const dNasc = docAluno.data().nascimento || docAluno.data().dataNascimento;
                    if (dNasc) {
                        const hojeData = new Date();
                        const nascData = new Date(dNasc);
                        if (hojeData.getDate() === nascData.getDate() && hojeData.getMonth() === nascData.getMonth()) {
                            listaAlertas.push({ id: docAluno.id, nome: docAluno.data().nome, turma: t, msg: `🎉 Está de parabéns hoje!`, tipo: 'aniversario' });
                        }
                    }
                }
            } catch(e) {}
        ...
        if (listaAlertas.length === 0) {
            ...
            listaAlertas.forEach(a => {
                let cor = a.tipo === 'falta' ? 'var(--danger-red)' : (a.tipo === 'prhf' ? 'var(--warning-yellow)' : '#b82bf2');
                let icon = a.tipo === 'falta' ? '<i class="fa-solid fa-user-xmark"></i>' : (a.tipo === 'prhf' ? '<i class="fa-solid fa-file-medical"></i>' : '<i class="fa-solid fa-cake-candles"></i>');
                alertasHtml += `
                <div onclick="window.abrirPerfil360Aluno('${a.id}')" style="cursor:pointer; transition:0.2s; display:flex; justify-content:space-between; align-items:center; background:rgba(0,0,0,0.2); padding:10px; border-radius:8px; border-left:3px solid ${cor}; margin-bottom:5px;" onmouseover="this.style.background='rgba(255,255,255,0.05)'" onmouseout="this.style.background='rgba(0,0,0,0.2)'">
                    ...
                    <div style="font-size:0.8rem; color:${cor}; display:flex; align-items:center; gap:5px;">${icon} ${a.msg}</div>
                </div>`;
            });
        }
        if(alCont) alCont.innerHTML = alertasHtml;

        // LER ALERTAS DO DIRETOR DE TURMA (As 4 Caixas)
        const avisosCont = document.getElementById('avisos-dt-container');
        if (avisosCont) {
            try {
                // Vai ler os alertas ativos guardados no perfil do professor
                const docAvisos = await getDoc(doc(db, "utilizadores", state.myUserId, "alertas", "dt"));
                const avisos = docAvisos.exists() ? docAvisos.data() : {};
                
                const buildCaixa = (nome, ativo, icon, cor) => `
                    <div style="background: rgba(0,0,0,0.2); border: 1px solid ${ativo ? cor : '#333'}; box-shadow: ${ativo ? `0 0 10px ${cor}40` : 'none'}; border-radius: 8px; padding: 10px; display: flex; align-items: center; gap: 10px; opacity: ${ativo ? '1' : '0.4'}; transition: 0.3s;">
                        ...
                        <div>
                            <strong style="color: ${ativo ? 'white' : 'var(--text-muted)'}; font-size: 0.85rem; display:block;">${nome}</strong>
                            <span style="color: ${ativo ? cor : 'var(--text-muted)'}; font-size: 0.7rem;">${ativo ? 'Pendente!' : 'Em dia'}</span>
                        </div>
                    </div>`;

                avisosCont.innerHTML = `
                    ${buildCaixa('Sínteses', avisos.sinteses, 'fa-solid fa-clipboard', '#10b981')}
                    ${buildCaixa('PCT', avisos.pct, 'fa-solid fa-users-rectangle', '#b82bf2')}
                    ${buildCaixa('Lançar Notas', avisos.notas, 'fa-solid fa-star', '#f59e0b')}
                    ${buildCaixa('Sumários', avisos.sumarios, 'fa-solid fa-book-open', '#3b82f6')}
                `;
            } catch(e) { avisosCont.innerHTML = ''; }
        }
...
export async function abrirPerfil360Aluno(alunoId) {
    ...
    if (discSelect) {
        ...
        discSelect.onchange = async () => {
            ...
            // Gráfico antigo caso os botões existam
            if (typeof desenharGraficoAluno === 'function' && btnGraphDisc) {
                desenharGraficoAluno(btnGraphDisc.classList.contains('active') ? 'disc' : 'global');
            }
            
            // Histórico de Notas da Disciplina
    ...
    if (elMomento) {
        ...
        elMomento.value = 'momento_1'; // Inicia com o 1º momento
    }
    ...
    try {
        ...
        // Obter todas as notas para RAM (Ler de "avaliacoes" e "notas" por precaução com a DB antiga)
        ...
        // 5. CÁLCULO DA MÉDIA DA DISCIPLINA PRINCIPAL E NOVO GRÁFICO
        const disciplinaPrimaria = isDT ? ordemDisciplinasGlobal[0] : state.disciplinasProfessor[0];
        let somaMedia = 0; let countNotas = 0; let notasParaGrafico = [];
        
        state.notasAlunoRAM.forEach(n => {
            if(n.disciplina === disciplinaPrimaria && n.notaOriginal !== 'REP' && n.valor > 0) {
                somaMedia += n.valor;
                countNotas++;
        ...
        if (elMedia) {
            if (countNotas > 0) elMedia.innerText = (somaMedia / countNotas).toFixed(1);
            else elMedia.innerText = '-';
        }

        // Gráfico Blindado
        const ctxEl = document.getElementById('chartEvolucaoAluno');
        if (ctxEl) {
            notasParaGrafico.sort((a,b) => a.modulo - b.modulo);
