...
function esconderTudoMenos(ecraAtivo) {
    [classHubView, classView, studentDetailView, viewAvaliacoes, viewDisciplinaModulos, 
     ...
     viewClassHorario, viewClassForum, viewClassEstatisticas].forEach(el => { if(el) el.style.display = 'none'; });
    ...
    if (user) {
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                if(dados.papel !== 'dt' && dados.papel !== 'admin') {
                    window.location.href = "index.html"; 
                    return;
                }
                
                myUserName = dados.nome.split(' ')[0];
                turmaAtual = dados.turma; // Assumimos que o DT tem a turma no perfil
                
                document.getElementById('header-user-name-staff').innerText = `Olá, ${myUserName} (DT)`;
                document.getElementById('class-hub-title').innerHTML = `A Minha Turma: ${turmaAtual}`;
    ...
    try {
        ...
        container.querySelectorAll('.btn-ver-aluno').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                // Carregar foto de perfil no detalhe
...
async function carregarHorario() {
    ...
    for(let i=0; i<5; i++) {
        ...
        mapDiasParaDataReal[diasUIAbrv[i]] = dStr; document.querySelectorAll(`.horario-slot[data-dia="${diasUIAbrv[i]}"]`).forEach(s => s.setAttribute('data-datareal', dStr));
...
async function calcularEstatisticasTurma() {
    ...
    try {
        ...
        if(snapshotAlunos.empty) { tabelaAlunos.innerHTML = '<tr><td colspan="4" class="text-muted center">Sem alunos registados nesta turma.</td></tr>'; return; }
        ...
        for(let alunoDoc of snapshotAlunos.docs) {
            ...
            try {
                ...
                prhfs.forEach(p => { 
                    const dataP = p.data(); if(dataP.status === 'ativa') prhfsAtivosGerais++; 
                    hPrhfPlan += (dataP.horasPresenciais || 0); if(dataP.registosManuais) { dataP.registosManuais.forEach(r => hPrhfCump += r.horas); }
                });
            } catch(subErr) {}
        ...
        document.getElementById('stat-faltas-totais').innerText = totalHorasFaltasTurma;
...
document.getElementById('btn-novo-forum')?.addEventListener('click', async () => { 
    ...
    const cList = document.getElementById('novo-forum-membros-list'); cList.innerHTML = '<p class="text-muted" style="text-align:center;">A procurar...</p>';
    try {
        const qAlunos = query(collection(db, "utilizadores"), where("turma", "==", turmaAtual), where("papel", "==", "aluno"));
        const snapshot = await getDocs(qAlunos); 
        let h = ''; snapshot.forEach(d => { h += `<label class="membro-checkbox-item"><input type="checkbox" class="cb-membro-forum" value="${d.id}" checked> ${d.data().nome}</label>`; });
        cList.innerHTML = h || '<p class="text-muted" style="text-align:center;">Sem alunos registados.</p>';
    } catch(e) {}
});
...
document.getElementById('btn-hub-informacoes')?.addEventListener('click', async () => { 
