...
document.getElementById('ee-upload-atestado')?.addEventListener('change', (e) => { const file = e.target.files[0]; if(!file) return; if(file.size > 2097152) { alert("Ficheiro demasiado grande! Tente tirar uma foto com menos resolução."); return; } document.getElementById('ee-atestado-nome').innerText = "Ficheiro anexado: " + file.name; document.getElementById('ee-atestado-nome').style.color = "var(--success-green)"; const reader = new FileReader(); reader.onload = (ev) => { comprovativoBase64 = ev.target.result; }; reader.readAsDataURL(file); });
