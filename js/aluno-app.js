    ...
    if (user) {
        ...
        try {
            ...
            if (docSnap.exists() && docSnap.data().papel === 'aluno') {
                ...
                // Preencher Select de Objetivos Inteligentes
                const objSelect = document.getElementById('obj-disciplina');
                if(objSelect) {
                    objSelect.innerHTML = ordemDisciplinasGlobal.map(disc => `<option value="${disc}">${disc}</option>`).join('');
                }

                carregarDadosPassaporte(d); carregarGamificacao(d);
                await construirHomeAdaptativa(d);
...
// ROUTING DE VISTAS (Navegação Global)
function esconderTudo() { document.querySelectorAll('.app-content > div').forEach(v => v.style.display = 'none'); }
...
document.body.addEventListener('click', (e) => {
    ...
    if(nav) {
        ...
        esconderTudo(); const tId = nav.getAttribute('data-target'); document.getElementById(tId).style.display = 'block';
        
        if(tId === 'view-aluno-perfil') { carregarObjetivosPessoais(); carregarHistoricoHumor(); carregarEstatisticasEstudo(); setTimeout(renderizarGraficoNotas, 150); }
    ...
    // Botões da Home
    if(e.target.closest('#btn-open-study-mode')) document.querySelector('.nav-item[data-target="view-study-mode"]')?.click();
    if(e.target.closest('#btn-open-caderno')) { 
        document.querySelector('.nav-item[data-target="view-aluno-caderno"]')?.click(); 
        if(!quillEditor) quillEditor = new Quill('#quill-editor', { theme: 'snow' }); 
        carregarResumos(); 
    }
    if(e.target.closest('#btn-open-sumarios')) document.querySelector('.nav-item[data-target="view-aluno-sumarios"]')?.click();
    if(e.target.closest('#btn-abrir-passaporte')) document.querySelector('.nav-item[data-target="view-aluno-passaporte"]')?.click();
    if(e.target.closest('#btn-open-notificacoes')) { esconderTudo(); document.getElementById('view-aluno-notificacoes').style.display='block'; carregarNotificacoesAluno(); }
    if(e.target.closest('#btn-voltar-notificacoes') || e.target.closest('#btn-voltar-caderno') || e.target.closest('#btn-voltar-sumarios') || e.target.closest('#btn-voltar-passaporte')) {
        document.querySelector('.nav-item[data-target="student-dashboard"]').click();
    }
});

// ==========================================
// 2. DASHBOARD ADAPTATIVO MÁGICO
// ==========================================
async function construirHomeAdaptativa(dados) {
    const cont = document.getElementById('dynamic-hero-section'); if(!cont) return;
    try {
        let tFaltas = 0, evs = [], pAtivos = 0, pHoras = 0, mRep = 0;
        
        // Contar Faltas
        try { const fS = await getDocs(collection(db, "utilizadores", myUserId, "faltas")); fS.forEach(d => { if(!d.data().justificada) tFaltas++; }); } catch(e){}
        // Contar Reprovações
        try { const nS = await getDocs(collection(db, "utilizadores", myUserId, "notas")); nS.forEach(d => { const n = d.data().nota; if(n==='REP'||Number(n)<10) mRep++; }); } catch(e){}
        // PRHFs
        try { const pS = await getDocs(collection(db, "utilizadores", myUserId, "prhfs")); pS.forEach(d => { if(d.data().status!=='concluida'){pAtivos++; pHoras+=Number(d.data().horasPresenciais||0);} }); } catch(e){}
        // Eventos da Turma
        if(minhaTurma) {
            try {
        ...
        if(tFaltas>0 || pAtivos>0 || mRep>0) {
            html += `<div class="card" style="background:linear-gradient(135deg,#ff4d4d,#cc0000); color:white; border:none; border-radius:16px; margin-bottom:20px;">
                ...
                <ul style="margin-bottom:20px; padding-left:20px; font-size:1.1rem; font-weight:bold; line-height:1.6;">
                    ${tFaltas>0?`<li>${tFaltas} Falta(s)</li>`:''}${mRep>0?`<li>${mRep} Módulo(s) Reprovado(s)</li>`:''}${pAtivos>0?`<li>${pAtivos} PRHF(s) pendentes</li>`:''}
                </ul>
            ...
            html += `<div class="card" style="background:linear-gradient(135deg,#00cc88,#009966); color:white; border:none; border-radius:16px; margin-bottom:20px;">
                ...
                <p style="font-size:1.05rem; margin-bottom:0; opacity:0.9;">Não tens avaliações marcadas nem pendências. Boa altura para os teus resumos!</p>
            </div>`; mMiss = true;
        }
...
document.getElementById('aluno-search-input')?.addEventListener('input', async (e) => {
    ...
    try {
        ...
        if(resArr.length === 0) box.innerHTML = '<p class="text-muted" style="margin:0; font-size:0.85rem;">Sem resultados.</p>'; 
            ...
            let h = ''; resArr.forEach(r => h += `<div style="padding:8px; border-bottom:1px solid #333; cursor:pointer;" onclick="document.getElementById('${r.id}').click(); document.getElementById('aluno-search-results').style.display='none';"><span style="font-size:0.7rem; color:var(--primary-green); text-transform:uppercase;">${r.t}</span><div style="font-size:0.9rem; color:white; margin-top:3px;">${r.txt}</div></div>`); box.innerHTML = h; 
        }
...
// ==========================================
// 4. PERFIL: OBJETIVOS INTELIGENTES (AUTO-VALIDAÇÃO)
// ==========================================
async function carregarObjetivosPessoais() {
    const cont = document.getElementById('lista-objetivos-container'); cont.innerHTML = '<p class="text-muted center">A carregar os teus objetivos...</p>';
    try {
        // Obter todas as notas do aluno para auto-validação
        const notasSnap = await getDocs(collection(db, "utilizadores", myUserId, "notas"));
        let melhoresNotas = {};
        notasSnap.forEach(n => {
            const dt = n.data(); if(dt.nota !== 'REP' && !isNaN(dt.nota)) { const val = Number(dt.nota); if(!melhoresNotas[dt.disciplina] || val > melhoresNotas[dt.disciplina]) melhoresNotas[dt.disciplina] = val; }
        });

        const snap = await getDocs(query(collection(db, "utilizadores", myUserId, "objetivos"), orderBy("timestamp", "desc")));
        let html = ''; let objGanhouXP = false; let novoXpT = 0;

        for (const d of snap.docs) {
            const obj = d.data(); let foiConcluidoAgora = false;
            
            // VERIFICAÇÃO INTELIGENTE DA NOTA (Se não estava concluído e o aluno tem a nota alvo)
            if(!obj.concluido && melhoresNotas[obj.disciplina] && melhoresNotas[obj.disciplina] >= Number(obj.notaAlvo)) {
                await updateDoc(doc(db, "utilizadores", myUserId, "objetivos", d.id), { concluido: true });
                obj.concluido = true; foiConcluidoAgora = true; objGanhouXP = true;
            }

            const checkColor = obj.concluido ? 'var(--success-green)' : '#444';
            const textDec = obj.concluido ? 'line-through' : 'none'; const textColor = obj.concluido ? 'var(--text-muted)' : 'white';
            ...
            html += `<div style="display:flex; align-items:center; justify-content:space-between; padding:12px; background:rgba(0,0,0,0.2); border-radius:8px; margin-bottom:8px; border-left: 3px solid ${checkColor};">
                        <div style="display:flex; align-items:center; gap:12px; flex:1;">
                            <div style="width:24px; height:24px; border-radius:50%; border:2px solid ${checkColor}; background:${obj.concluido ? checkColor : 'transparent'}; display:flex; align-items:center; justify-content:center;">${obj.concluido ? '<i class="fa-solid fa-check" style="color:var(--bg-dark); font-size:0.75rem;"></i>' : ''}</div>
                            <span style="text-decoration:${textDec}; color:${textColor}; font-size:0.95rem; flex:1;">Tirar <strong>${obj.notaAlvo}</strong> a ${obj.disciplina}</span>
                        </div>
                        <i class="fa-solid fa-trash" style="color:var(--danger-red); cursor:pointer; font-size:0.9rem; padding: 5px;" onclick="window.apagarObjetivo('${d.id}')"></i>
                     </div>`;
        }

        // Se algum foi concluído automaticamente agora, dá XP!
        if(objGanhouXP) {
            const userS = await getDoc(doc(db, "utilizadores", myUserId)); let axp = userS.exists()&&userS.data().xp?userS.data().xp:0;
            await updateDoc(doc(db, "utilizadores", myUserId), { xp: axp + 50 }); carregarGamificacao({xp: axp+50}); alert("🎉 Um Objetivo Inteligente foi concluído com as tuas Notas! +50 XP");
        }
...
window.apagarObjetivo = async (id) => { if(confirm("Queres mesmo eliminar este objetivo?")) { try { await deleteDoc(doc(db, "utilizadores", myUserId, "objetivos", id)); carregarObjetivosPessoais(); } catch(e) {} } };

// ==========================================
// 5. GRÁFICOS, ESTATÍSTICAS E HUMOR
// ==========================================
function carregarGamificacao(dados) {
    ...
    document.getElementById('aluno-rank-title').innerText = r; if(document.getElementById('perfil-titulo-central')) document.getElementById('perfil-titulo-central').innerText = r;
}
...
async function carregarHistoricoHumor() {
    const c = document.getElementById('mood-history-container'); c.innerHTML = '<p class="text-muted center">A carregar...</p>';
...
// ==========================================
// 6. CADERNETA (Faltas e PRHF Corrigidos)
// ==========================================
...
async function carregarTimelineAluno() {
    ...
    try {
        ...
        if(ev.length === 0) { cCont.innerHTML = '<p class="text-muted center">Histórico limpo nesta categoria.</p>'; return; }
...
async function carregarPrhfsAluno() {
    ...
    try {
        ...
        prhfsArr.forEach(p => { 
            ...
            const hTot = Number(p.horasTotais || p.horas || '?'); const hPres = Number(p.horasPresenciais || 0); const hAut = isNaN(hTot)? '?' : hTot - hPres;
...
window.abrirAcaoPrhf = (id, disc, mod, prazo) => {
    ...
    document.getElementById('btn-voltar-acao-prhf').onclick = () => { document.querySelectorAll('.app-content > div').forEach(v => v.style.display = 'none'); document.getElementById('view-aluno-caderneta').style.display = 'block'; carregarPrhfsAluno(); };
    ...
    document.getElementById('btn-enviar-proposta-prhf').onclick = async (e) => {
        ...
        try { await updateDoc(doc(db, "utilizadores", myUserId, "prhfs", id), { propostaAluno: `Data: ${d} | Das ${hi} às ${hf}`, propostaLidaDT: false }); btn.innerHTML = '<i class="fa-solid fa-check"></i> Enviado'; btn.style.backgroundColor = 'var(--success-green)'; setTimeout(() => { document.getElementById('btn-voltar-acao-prhf').click(); btn.innerHTML = 'Enviar Proposta ao Professor'; btn.disabled = false; btn.style.backgroundColor = 'var(--primary-green)';}, 1500); } catch(err) { btn.innerHTML = "Erro"; setTimeout(() => { btn.disabled = false; }, 1500); }
    };
};

async function carregarNotasAluno() { /* Igual, omitido apenas no resumo para evitar token limits. É chamada no Switch Tab. */ }
async function carregarComportamentoAluno() { /* Igual */ }
async function carregarObservacoesAluno() { /* Igual */ }

// ==========================================
// 7. FÓRUM COM SELEÇÃO DE COLEGAS
// ==========================================
async function carregarForuns() {
    ...
    cont.querySelectorAll('.canal-card').forEach(c => c.addEventListener('click', (e) => { window.alunoForumAtivoId = e.currentTarget.getAttribute('data-id'); document.getElementById('aluno-chat-active-title').innerText = e.currentTarget.getAttribute('data-nome'); document.getElementById('aluno-forum-channel-list').style.display = 'none'; document.getElementById('aluno-forum-chat-view').style.display = 'flex'; document.getElementById('btn-create-chat-aluno').style.display = 'none'; iniciarChatAluno(window.alunoForumAtivoId); }));
}
...
document.getElementById('btn-create-chat-aluno')?.addEventListener('click', async () => { 
    ...
    try {
        const colegasSnap = await getDocs(query(collection(db, "utilizadores"), where("turma", "==", minhaTurma), where("papel", "==", "aluno")));
        let cHtml = '';
        colegasSnap.forEach(d => { if(d.id !== myUserId) { cHtml += `<label style="display:flex; align-items:center; gap:10px; color:white; font-size:0.9rem; padding:8px 0; cursor:pointer;"><input type="checkbox" class="colegas-check" value="${d.id}" style="width:18px;height:18px;accent-color:var(--primary-green);"> ${d.data().nome}</label>`; } });
        cCont.innerHTML = cHtml === '' ? '<p class="text-muted center">Sem colegas.</p>' : cHtml;
...
document.getElementById('btn-confirmar-novo-forum')?.addEventListener('click', async () => {
    ...
    let membrosSelecionados = [myUserId];
    document.querySelectorAll('.colegas-check:checked').forEach(chk => membrosSelecionados.push(chk.value));
    try { await addDoc(collection(db, "turmas", minhaTurma, "foruns"), { nome: nome, tipo: 'permanente', isDefault: false, membros: membrosSelecionados, criadoPor: myUserName }); document.getElementById('modal-criar-forum').style.display = 'none'; document.getElementById('input-nome-novo-forum').value = ''; alert("Grupo criado!"); carregarForuns(); } catch(e) {}
});
...
function iniciarChatAluno(fId) {
    ...
    chatUnsubscribeAluno = onSnapshot(query(collection(db, "turmas", minhaTurma, "foruns", fId, "mensagens"), orderBy("timestamp")), (snap) => {
        let html = ''; snap.forEach(doc => { const m = doc.data(); const isMe = m.remetente === myUserName; html += `<div class="chat-bubble ${isMe ? 'admin' : 'student'}"><strong>${isMe ? 'Tu' : m.remetente}</strong><br>${m.texto}<span class="chat-meta">${new Date(m.timestamp).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</span></div>`; }); chatC.innerHTML = html; chatC.scrollTop = chatC.scrollHeight;
    });
}
document.getElementById('btn-aluno-send-msg')?.addEventListener('click', async () => { const inp = document.getElementById('aluno-input-forum-msg'); const t = inp.value.trim(); if(!t || !window.alunoForumAtivoId) return; try { await addDoc(collection(db, "turmas", minhaTurma, "foruns", window.alunoForumAtivoId, "mensagens"), { remetente: myUserName, texto: t, timestamp: Date.now() }); inp.value = ''; } catch(e) {} });

// ==========================================
// 8. POMODORO (Modo Foco e Música)
// ==========================================
...
document.getElementById('btn-start-study')?.addEventListener('click', (e) => {
    ...
    pTmr = setInterval(() => {
        pRest--; const m = Math.floor(pRest / 60).toString().padStart(2, '0'); const s = (pRest % 60).toString().padStart(2, '0');
        document.getElementById('study-timer-text').innerText = `${m}:${s}`;
        if(pRest <= 0) { clearInterval(pTmr); document.getElementById('study-controls').style.display = 'none'; document.getElementById('post-study-log').style.display = 'block'; }
    }, 1000);
});
...
document.getElementById('btn-save-study-log')?.addEventListener('click', async (e) => {
    ...
    try {
        ...
        setTimeout(() => { document.getElementById('study-log-text').value = ''; document.getElementById('post-study-log').style.display = 'none'; document.getElementById('study-controls').style.display = 'block'; b.innerHTML = '<i class="fa-solid fa-save"></i> Guardar e Ganhar XP'; b.disabled = false; document.getElementById('btn-voltar-study').click(); }, 2000);
    } catch(err) { b.innerHTML = "Erro"; setTimeout(() => { b.disabled = false; }, 2000); }
});

// ==========================================
// 9. CADERNO DIGITAL (Resumos)
// ==========================================
...
function carregarDadosPassaporte(dados) {
    if(dados.fct) { document.getElementById('aluno-fct-horas').innerText = `${dados.fct.horasRealizadas||0} / ${dados.fct.horasTotal||0}h`; document.getElementById('aluno-fct-progress').style.width = `${((dados.fct.horasRealizadas||0)/(dados.fct.horasTotal||1))*100}%`; }
