            ...
            if (docSnap.exists()) {
                ...
                if (state.myRoles.some(r => ['professor', 'diretor_turma', 'orientador_pap', 'coordenador'].includes(r))) {
                    ...
                    state.turmasProfessor.forEach(t => { 
                        let ano = parseInt(t.match(/\d+/)?.[0]) || 10; 
...
document.getElementById('forum-turma-select')?.addEventListener('change', async (e) => { 
    ...
    try { 
        ...
        arr.sort((a,b) => a.nome.localeCompare(b.nome)); // ORDEM ALFABÉTICA
        ...
        cCont.innerHTML = '<p class="text-danger center" style="grid-column: span 2;">Erro.</p>'; 
    } 
});

// Evento Global para captar mudanças nas Checkboxes (Para colorir os labels do Fórum)
...
// ========================================================
// LISTENER GLOBAL DE CLIQUES (Totalmente Descomprimido)
// ========================================================
document.body.addEventListener('click', async (e) => {
    ...
    // Fechar Dropdown do Perfil e Modal FAB se clicar fora
    ...
    // Logout
    if (e.target.closest('#btn-logout-dropdown')) {
        signOut(auth);
    }

    // Botões Universais Fechar Modal
    ...
    // Fórum Criar: Deselecionar Todos
    ...
    // Fórum Editar: Deselecionar Todos
    ...
    if (e.target.closest('.btn-edit-chat')) { 
        ...
        try { 
            ...
            mCont.innerHTML = '<p class="text-danger center" style="grid-column: span 2;">Erro.</p>'; 
        } 
        return; 
    }
    ...
    if (e.target.closest('#btn-nova-sessao-pap')) { 
        prepararModalNovaSessao(); 
        return; 
    }


    // ========================================================
    // FAB - ABERTURA DOS MODAIS PRINCIPAIS
    // ========================================================
    
    if (e.target.closest('#btn-fab-falta')) { 
        document.getElementById('modal-fab-menu').style.display = 'none'; 
        document.getElementById('erro-modal-faltas').style.display = 'none'; 
        document.getElementById('global-falta-turma-container').style.display = 'block'; 
        ...
        document.getElementById('lancar-falta-disciplina').style.display = state.disciplinasProfessor.length > 1 ? 'block' : 'none'; 
    ...
    if (e.target.closest('#btn-fab-ocorrencia')) { 
        ...
        const selTurmas = document.getElementById('oco-global-turma');
        selTurmas.innerHTML = '<option value="">Selecionar Turma...</option>' + state.turmasProfessor.map(t => `<option value="${t}">Turma ${t}</option>`).join(''); 
    ...
    if (e.target.closest('#btn-fab-sumario')) { 
        ...
        const selTurmas = document.getElementById('lancar-sumario-turma');
        selTurmas.innerHTML = '<option value="">Selecionar Turma...</option>' + state.turmasProfessor.map(t => `<option value="${t}">Turma ${t}</option>`).join(''); 
    ...
    // ========================================================
    // PLANTA DA SALA - CÉREBRO HÍBRIDO
    // ========================================================
    ...
    if (e.target.closest('#btn-apagar-forum')) { 
        if(confirm("Apagar definitivamente este grupo de chat?")) { 
            ...
            try { 
                ...
                document.getElementById('modal-editar-forum').style.display = 'none'; 
                btn.innerHTML = '<i class="fa-solid fa-trash"></i> Apagar Chat Definitivamente'; 
                btn.disabled = false; 
                carregarForunsProf(); 
            } catch(err) { 
                btn.innerHTML = 'Erro!'; 
    ...
    if (e.target.closest('#btn-confirm-novo-forum')) { 
        ...
        btnConf.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>'; 
    ...
    if (e.target.closest('#btn-abrir-copiloto')) { 
        ...
        if (isDT) { 
            ...
            promptText = `Atua como professor da disciplina de ${document.getElementById('perfil-disc-select').value || 'Técnicas'}. Escreve uma síntese de avaliação para o aluno ${alunoNome}. A síntese deve cumprir as regras da direção: Ser objetiva e construtiva; 1. Elogiar pontos fortes. 2. Apontar aspetos a melhorar na disciplina. 3. Dar estratégias claras de progressão. (Nota: O aluno concluiu ${modFeitos} módulos e tem ${faltas}h de faltas na globalidade). Tom: Profissional e direto ao assunto.`; 
        } 
    ...
    if (e.target.closest('#btn-copiar-prompt')) { 
        ...
        setTimeout(() => { 
            ...
            document.getElementById('modal-copiloto-ia').style.display = 'none'; 
        }, 1500); 
        return; 
    }
    ...
    if (e.target.closest('.btn-download-ics')) { 
        ...
        a.href = url; 
        a.download = `${tit.replace(/\s+/g,'_')}.ics`; 
    ...
    if (e.target.closest('#btn-gravar-novo-prhf')) { 
        ...
        if (!tAluno || !tDisc || !tMod || !tHorasT || !tPrazo || !tDesc) { 
        ...
        try { 
            ...
            b.innerHTML = '<i class="fa-solid fa-check"></i> Atribuído'; 
            setTimeout(() => { 
                b.innerHTML = '<i class="fa-solid fa-paper-plane"></i> Atribuir PRHF'; 
                b.disabled = false; 
                document.getElementById('modal-criar-prhf').style.display = 'none'; 
                carregarTarefasProf(); 
                analisarEAtualizarTurma(state.selectedTurma); 
            }, 1500); 
        } catch (err) { 
            errDiv.innerText = "Erro ao gravar. Tenta de novo."; 
            errDiv.style.display = 'block'; 
            b.innerHTML = '<i class="fa-solid fa-paper-plane"></i> Atribuir PRHF'; 
            b.disabled = false; 
        } 
