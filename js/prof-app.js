...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        console.log("Firebase Auth OK. User:", state.myUserId);
        try {
            ...
            if (docSnap.exists()) {
                ...
                if (state.myRoles.some(r => ['professor', 'diretor_turma', 'orientador_pap', 'coordenador'].includes(r))) {
                    ...
                    const nameLabel = document.getElementById('header-user-name-prof');
                    if(nameLabel) nameLabel.innerText = state.myUserName;
                    ...
                    const dropList = document.getElementById('lista-capas-dropdown');
                    if(dropList) dropList.innerHTML = dropdownHtml;
                    ...
                    window.mudarCapaProfessor = (novoPapel) => {
                        ...
                        const dropMenu = document.getElementById('dropdown-perfis');
                        if(dropMenu) dropMenu.style.display = 'none';
                        const homeBtn = document.querySelector('.nav-item[data-target="view-prof-dashboard"]');
                        if(homeBtn) homeBtn.click();
                    };

                    window.mudarCapaProfessor('professor');
                    
                    const btnToggle = document.getElementById('btn-toggle-perfis');
                    if(btnToggle) {
                    ...
                    if(document.getElementById('perfil-nome-prof-view')) document.getElementById('perfil-nome-prof-view').innerText = state.myUserName;
                    if(document.getElementById('perfil-disciplinas-lista')) document.getElementById('perfil-disciplinas-lista').innerText = state.disciplinasProfessor.length > 0 ? state.disciplinasProfessor.join(' • ') : 'Nenhuma disciplina configurada.';
                    if(document.getElementById('perfil-papeis-lista')) document.getElementById('perfil-papeis-lista').innerText = state.myRoles.map(r => r.toUpperCase().replace('_', ' ')).join(' • ');
                    ...
                    if(document.getElementById('header-avatar-circle')) document.getElementById('header-avatar-circle').innerHTML = `<img src="${fotoUrl}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`;
                    if(document.getElementById('prof-avatar-img')) document.getElementById('prof-avatar-img').src = fotoUrl;
                ...
                console.error("Documento do utilizador não encontrado no Firebase!");
            }
        } catch (e) { 
...
document.body.addEventListener('change', async (e) => {
    ...
    if (e.target.id === 'forum-turma-select') {
        ...
        if (!t) { 
            if(cCont) cCont.innerHTML = '<p class="text-muted center" style="font-size:0.8rem; grid-column: span 2;">Seleciona uma turma primeiro.</p>'; 
