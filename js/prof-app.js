    ...
    if (user) {
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                if (profData.papel && !myRoles.includes(profData.papel)) {
                    myRoles.push(profData.papel);
                }
...
document.getElementById('mat-file')?.addEventListener('change', (e) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
        alert("O ficheiro é muito grande. O limite máximo é 5MB.");
        return;
    }
    ...
    reader.onload = (event) => {
        materialBase64 = event.target.result;
    };
...
document.body.addEventListener('click', async (e) => {
    ...
    if (e.target.closest('#tab-tarefas-prhf')) {
        ...
        e.target.closest('.falta-tab-btn').classList.add('active');
        document.getElementById('sec-tarefas-prhf').style.display = 'block';
        document.getElementById('sec-tarefas-passaporte').style.display = 'none';
        carregarTarefasProf();
    }
    ...
    if (e.target.closest('#tab-tarefas-passaporte')) {
        document.querySelectorAll('.falta-tab-btn').forEach(b => b.classList.remove('active'));
        e.target.closest('.falta-tab-btn').classList.add('active');
        document.getElementById('sec-tarefas-prhf').style.display = 'none';
        document.getElementById('sec-tarefas-passaporte').style.display = 'block';
        carregarTarefasProf();
    }
    ...
    if (e.target.closest('#btn-gravar-ocorrencia')) {
        ...
        const b = e.target.closest('#btn-gravar-ocorrencia');
        ...
        try {
            if (tipo === 'positiva') {
                ...
                let axp = uS.exists() && uS.data().xp ? uS.data().xp : 0;
                await addDoc(collection(db, "utilizadores", alunoSelecionadoId, "ocorrencias"), { 
                    titulo: "Reconhecimento Positivo", 
                    descricao: motivo, 
                    tipo: "positiva", 
                    autor: myUserName, 
                    timestamp: Date.now(), 
            ...
            setTimeout(() => { 
                b.innerHTML = 'Confirmar Registo'; 
                b.disabled = false; 
                document.getElementById('modal-ocorrencia').style.display = 'none'; 
                document.getElementById('modal-perfil-aluno').style.display = 'none'; 
    ...
    if (e.target.closest('#btn-gravar-material')) {
        ...
        const b = e.target.closest('#btn-gravar-material');
        b.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>';
        b.disabled = true;
        ...
        try {
            await addDoc(collection(db, "turmas", selectedTurma, "sumarios"), { 
                ...
                ficheiroBase64: materialBase64, 
                timestamp: Date.now() 
            });
            
            b.innerHTML = '<i class="fa-solid fa-check"></i> Partilhado';
            b.style.backgroundColor = "var(--success-green)";
            
            setTimeout(() => { 
                b.innerHTML = '<i class="fa-solid fa-paper-plane"></i> Partilhar com a Turma'; 
                b.disabled = false; 
    ...
    if (e.target.closest('#btn-confirmar-rejeicao')) {
        ...
        const b = e.target.closest('#btn-confirmar-rejeicao');
        b.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>';
        b.disabled = true;
        ...
        try {
            await updateDoc(doc(db, "utilizadores", aId, "prhfs", pId), { 
                propostaLidaDT: true, 
                propostaAluno: null, 
                feedbackProfessor: "Proposta Rejeitada: " + feedback 
            });
            
            b.innerHTML = '<i class="fa-solid fa-check"></i> Rejeitado';
            setTimeout(() => { 
                b.innerHTML = 'Rejeitar Agendamento'; 
                b.disabled = false; 
                document.getElementById('modal-rejeitar-prhf').style.display = 'none'; 
                carregarTarefasProf(); 
            }, 1500);
    ...
    if (e.target.closest('#btn-modal-faltas')) {
        ...
        alunosTurmaRAM.forEach(al => { 
            h += `
            <label style="display:flex; justify-content:space-between; align-items:center; padding:10px; background:rgba(255,255,255,0.05); margin-bottom:5px; border-radius:8px; border:1px solid #333; cursor:pointer;">
        ...
        c.innerHTML = h; 
        document.getElementById('falta-horas-bloco').value = ''; 
    ...
    if (e.target.closest('#btn-confirmar-faltas')) {
        ...
        const b = e.target.closest('#btn-confirmar-faltas'); 
        b.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>'; 
        b.disabled = true;
        ...
        for (const chk of ausentes) { 
            await addDoc(collection(db, "utilizadores", chk.value, "faltas"), { 
                disciplina: "Geral", 
                horas: Number(horas), 
                dataInicio: new Date().toISOString().split('T')[0], 
                justificada: false, 
                criadoPor: myUserName, 
                criadoEm: new Date().toISOString() 
            }); 
        }
        
        b.innerHTML = '<i class="fa-solid fa-check"></i> Faltas Registadas'; 
        b.style.backgroundColor = "var(--success-green)"; 
        
        setTimeout(() => { 
            b.innerHTML = 'Gravar Registo'; 
            b.disabled = false; 
            b.style.backgroundColor = "var(--danger-red)"; 
    ...
    if (e.target.closest('#btn-modal-notas')) {
        ...
        alunosTurmaRAM.forEach(al => { 
            h += `
            <div style="display:flex; justify-content:space-between; align-items:center; padding:10px; background:rgba(255,255,255,0.05); border-radius:8px; border:1px solid #333;">
        ...
        try {
            for (const n of notasParaGravar) {
                ...
                if (n.nota !== 'REP' && !isNaN(n.nota) && Number(n.nota) >= 10) { 
                    ...
                    if(uS.exists()) { 
                        await updateDoc(doc(db, "utilizadores", n.id), { xp: (uS.data().xp || 0) + 20 }); 
                    } 
                }
            }
            
            b.innerHTML = '<i class="fa-solid fa-check"></i> Turma Atualizada!'; 
            b.style.backgroundColor = "var(--success-green)"; 
            b.style.color = "white";
            
            setTimeout(() => { 
                b.innerHTML = '<i class="fa-solid fa-save"></i> Gravar Notas na Turma'; 
                b.disabled = false; 
                b.style.backgroundColor = "var(--warning-yellow)"; 
                b.style.color = "black"; 
    ...
    if (e.target.closest('#btn-gravar-evento')) {
        ...
        const tp = document.getElementById('evento-tipo').value;
        if (!t || !d) return alert("Preenche Título e Data.");
        
        const b = e.target.closest('#btn-gravar-evento'); 
        b.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>'; 
        b.disabled = true;
        
        try {
            await addDoc(collection(db, "turmas", selectedTurma, "eventos"), { 
                titulo: t, 
                data: d, 
                tipo: tp, 
                professor: myUserName 
    ...
    if (e.target.closest('#btn-gravar-novo-prhf')) {
        ...
        const tAluno = document.getElementById('prhf-aluno').value; 
        ...
        const tHoras = document.getElementById('prhf-horas').value; 
        ...
        try {
            await addDoc(collection(db, "utilizadores", tAluno, "prhfs"), { 
                ...
                modulo: Number(tMod), 
                prazo: tPrazo, 
                horasPresenciais: Number(tHoras), 
                descricao: tDesc, 
                status: 'pendente', 
            ...
            b.innerHTML = '<i class="fa-solid fa-check"></i> Plano Ativado'; 
            b.style.backgroundColor = "var(--success-green)"; 
            
            setTimeout(() => { 
                b.innerHTML = '<i class="fa-solid fa-paper-plane"></i> Enviar para o Aluno'; 
                b.disabled = false; 
                b.style.backgroundColor = "var(--danger-red)"; 
    ...
    if (e.target.closest('.btn-aceitar-proposta')) {
        ...
        try { 
            await updateDoc(doc(db, "utilizadores", aId, "prhfs", pId), { 
                propostaLidaDT: true, 
                feedbackProfessor: "Proposta Aceite! Comparece no dia/hora combinados." 
            }); 
            carregarTarefasProf(); 
        } catch (err) {}
    }
    ...
    if (e.target.closest('.btn-concluir-prhf')) {
        ...
        const aId = btn.getAttribute('data-aluno'); 
        const pId = btn.getAttribute('data-prhf'); 
        const currXP = parseInt(btn.getAttribute('data-xp')) || 0;
        ...
        if (confirm("Confirma que o aluno concluiu a recuperação com sucesso? Ele receberá +100 XP.")) {
            ...
            try { 
                await updateDoc(doc(db, "utilizadores", aId, "prhfs", pId), { 
                    status: 'concluida', 
                    feedbackProfessor: "Recuperação concluída com sucesso." 
                }); 
    ...
    if (e.target.closest('#btn-salvar-obs-dt')) {
        ...
        try { 
            await setDoc(doc(db, "utilizadores", alunoSelecionadoId, "reunioes", "1_avaliacao"), { global: txt }, { merge: true }); 
            b.innerHTML = '<i class="fa-solid fa-check"></i> Gravado'; 
            b.style.backgroundColor = "var(--success-green)"; 
            
            setTimeout(() => { 
                b.innerHTML = 'Gravar Observação'; 
                b.disabled = false; 
                b.style.backgroundColor = "var(--primary-green)"; 
            }, 2000); 
    ...
    if (e.target.closest('#btn-justificar-faltas')) {
        ...
        if (confirm("Pretendes justificar todas as faltas pendentes deste aluno?")) {
            const b = e.target.closest('#btn-justificar-faltas'); 
            b.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>'; 
            b.disabled = true;
            ...
            try {
                ...
                for (const f of fS.docs) { 
                    await updateDoc(doc(db, "utilizadores", alunoSelecionadoId, "faltas", f.id), { 
                        justificada: true, 
                        justificadaPor: myUserName 
                    }); 
                }
                
                b.innerHTML = '<i class="fa-solid fa-check"></i> Faltas Justificadas'; 
                b.style.color = "var(--success-green)"; 
                b.style.borderColor = "var(--success-green)";
                
                setTimeout(() => { 
                    b.innerHTML = '<i class="fa-solid fa-clipboard-check"></i> Justificar Todas as Faltas Pendentes'; 
                    b.disabled = false; 
                    b.style.color = "#00d2ff"; 
                    b.style.borderColor = "#00d2ff"; 
                    abrirPerfil360Aluno(alunoSelecionadoId); 
                    analisarEAtualizarTurma(selectedTurma); 
                }, 2000);
    ...
    if (e.target.closest('.btn-validar-fct')) {
        ...
        try { 
            await updateDoc(doc(db, "utilizadores", aId), { "fct.validadoDT": true }); 
            carregarTarefasProf(); 
        } catch (err) { 
            btn.innerHTML = "Erro"; 
...
// ==========================================
// 4. LÓGICA DE SELECTS E CHANGE EVENTS
// ==========================================
...
document.getElementById('prhf-turma')?.addEventListener('change', async (e) => {
    const s = document.getElementById('prhf-aluno'); 
    const t = e.target.value;
    
    if (!t) { 
        s.innerHTML = '<option value="">Selecione primeiro a Turma</option>'; 
        return; 
    }
...
document.getElementById('evento-data')?.addEventListener('change', (e) => {
    ...
    const day = dateSel.getDay(); 
    const diff = dateSel.getDate() - day + (day == 0 ? -6 : 1);
    ...
    if (provasNaSemana >= 3) {
        document.getElementById('aviso-colisao-agenda').style.display = 'block'; 
    } else {
        document.getElementById('aviso-colisao-agenda').style.display = 'none';
    }
});
...
// ==========================================
// 5. CARREGAMENTO DE DADOS E RENDERIZAÇÃO
// ==========================================
async function analisarEAtualizarTurma(turmaId) {
    ...
    try {
        ...
        alunosTurmaRAM = []; 
        qAlunos.forEach(d => alunosTurmaRAM.push({ id: d.id, ...d.data() })); 
        alunosTurmaRAM.sort((a,b) => a.nome.localeCompare(b.nome));
        
        const qEvs = await getDocs(collection(db, "turmas", turmaId, "eventos")); 
        eventosTurmaRAM = []; 
        qEvs.forEach(d => eventosTurmaRAM.push(d.data()));

        let asstText = ""; 
        let alunosEmRisco = 0; 
        let totalPrhfsTurma = 0; 
        let htmlAlunos = '';
        ...
        for (let i=0; i<alunosTurmaRAM.length; i++) {
            ...
            fS.forEach(f => { if(!f.data().justificada) numFaltas++; });
            
            const pS = await getDocs(collection(db, "utilizadores", al.id, "prhfs")); 
            pS.forEach(p => { if(p.data().status !== 'concluida') numPrhfs++; });
            
            totalPrhfsTurma += numPrhfs; 
            let corBola = 'status-green';
            
            if (numFaltas > 5 || numPrhfs > 2) { 
                corBola = 'status-red'; 
                alunosEmRisco++; 
            } else if (numFaltas > 2 || numPrhfs > 0) { 
        ...
        listC.innerHTML = '<p class="text-danger center">Erro a processar turma.</p>'; 
    }
}

async function abrirPerfil360Aluno(alunoId) {
    alunoSelecionadoId = alunoId; 
    const al = alunosTurmaRAM.find(a => a.id === alunoId); 
    if (!al) return;
    ...
    try {
        ...
        fS.forEach(f => { if(!f.data().justificada) fCount++; });
        
        const pS = await getDocs(collection(db, "utilizadores", alunoId, "prhfs")); 
        pS.forEach(p => { if(p.data().status !== 'concluida') pCount++; });
        ...
        if (myRoles.includes('diretor_turma')) {
            ...
            if (rS.exists() && rS.data().global) {
                document.getElementById('p-aluno-obs-dt').value = rS.data().global; 
            } else {
                document.getElementById('p-aluno-obs-dt').value = '';
            }
        }
    } catch (e) {}
...
async function carregarRadarProfessor() {
    const pC = document.getElementById('radar-pendentes-container'); 
    const aC = document.getElementById('radar-agenda-container');
    
    pC.innerHTML = '<p class="text-muted center">A calcular...</p>'; 
    aC.innerHTML = '<p class="text-muted center">A calcular...</p>';
    
    if (turmasProfessor.length === 0) { 
        pC.innerHTML = '<p class="text-muted center">Nenhuma turma atribuída.</p>'; 
        aC.innerHTML = ''; 
        return; 
    }
    ...
    try {
        ...
        if (fut.length > 0) {
            ...
            fut.forEach(e => { 
                ah += `
                <div style="display:flex; justify-content:space-between; padding:10px; background:rgba(0,0,0,0.2); border-radius:8px; margin-bottom:5px; border-left:3px solid var(--primary-green);">
                    <div>
                        <strong style="color:white; font-size:0.9rem;">${e.titulo}</strong><br>
                        <span style="font-size:0.75rem; color:var(--text-muted);">Turma ${e.turma}</span>
                    </div>
            ...
            aC.innerHTML = '<p class="text-muted center" style="font-size:0.85rem;">Agenda livre para as próximas semanas.</p>'; 
        }
        
        pC.innerHTML = `
        <div style="padding:10px; border:1px dashed #333; border-radius:8px; text-align:center;">
            <p style="font-size:0.85rem; color:var(--text-muted); margin:0;">Tudo atualizado! Nenhuma tarefa urgente para hoje.</p>
        </div>`;
...
async function carregarTarefasProf() {
    ...
    if (isPRHFTab) {
        ...
        if (turmasProfessor.length === 0) { 
            container.innerHTML = '<p class="text-muted center">Sem turmas atribuídas.</p>'; 
            return; 
        }
        ...
        try {
            ...
            if (pendentes.length === 0) {
                h = `
                <div style="padding:15px; border:1px dashed var(--success-green); border-radius:8px; text-align:center;">
                    <p style="color:var(--success-green); font-size:0.9rem; margin:0;">Excelente! Não há Planos de Recuperação em curso nas tuas turmas.</p>
                </div>`;
            }
            ...
            pendentes.forEach(p => {
                ...
                if (p.propostaAluno && p.propostaLidaDT === false) { 
                    ...
                    <div style="background:rgba(255,204,0,0.1); border:1px dashed var(--warning-yellow); padding:10px; border-radius:8px; margin-top:10px;">
                        <strong style="color:var(--warning-yellow); font-size:0.85rem;"><i class="fa-solid fa-clock"></i> Proposta de Agendamento:</strong>
                    ...
                    acoesProposta = `
                    <div style="margin-top:10px; font-size:0.8rem; color:var(--success-green);">
                        <i class="fa-solid fa-check-double"></i> Agendamento confirmado com o Aluno.
                    </div>`; 
                }
                ...
                <div class="card" style="margin-bottom:15px; border-left: 4px solid var(--danger-red);">
                    <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                        ...
                        <button class="btn-concluir-prhf" data-aluno="${p.alunoId}" data-prhf="${p.id}" data-xp="${p.alunoXP}" style="background:var(--bg-dark); border:1px solid var(--success-green); color:var(--success-green); padding:5px 10px; border-radius:6px; cursor:pointer; font-size:0.8rem;">
                            <i class="fa-solid fa-check"></i> Concluir
                        </button>
                    </div>
                    <p style="font-size:0.85rem; color:var(--text-light); margin:10px 0;">${p.descricao}</p>
                    <div style="font-size:0.8rem; color:var(--text-muted);">
                        Prazo: <strong style="color:white;">${p.prazo.split('-').reverse().join('/')}</strong> | Horas: <strong>${p.horasPresenciais}h</strong>
                    </div>
                    ${acoesProposta}
                </div>`;
            }); 
            ...
            container.innerHTML = '<p class="text-danger center">Erro a carregar PRHFs.</p>'; 
        }
        ...
        try {
                ...
                if (al.fct && al.fct.horasRealizadas > 0) { 
                    if (al.fct.validadoDT) {
                        ...
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            <span style="color:var(--warning-yellow); font-size:0.8rem;">${al.fct.horasRealizadas}h declaradas</span> 
            ...
            container.innerHTML = '<p class="text-danger center">Erro ao carregar passaportes.</p>'; 
        }
    }
}
...
function carregarForunsProf() {
    ...
    cont.innerHTML = '<p class="text-muted center" style="font-size:0.85rem; margin-top:20px;">O Fórum das Disciplinas estará disponível na próxima atualização.</p>';
}
